...
class sysinfo {
	...
	function memory () {
		...
		$results['ram']['percent'] = ($results['ram']['total'] == 0) ? 0 : ceil( ( $results['ram']['used'] * 100 ) / $results['ram']['total'] );
		...
		$results['swap']['percent'] = ($results['swap']['total'] == 0 ) ? 0 : ceil( $results['swap']['used'] / $results['swap']['total'] * 100 );
		return $results;
	}
