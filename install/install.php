...
<body>
	...
	<div>
		<h2>To install PHPspeed follow each of the steps below</h2>

		<ol id="install">
			<li>
				Edit your <b>config_db.php</b> file before going to the next step. (You must manually make these changes)
				<table>
					 ...
					 <tr><td>$dbpass = "";</td><td>$dbpass = "<?php if ($dbpass != "" ) {echo "<small>hidden</small>"; } ?>";<?php if ($dbpass == "") { echo "<font color='red'><small>&nbsp;&nbsp;&nbsp;&laquo;&nbsp;EMPTY!!</small></font>"; } ?></td></tr>
					 <tr><td>$dbname = "phpspeed";</td><td>$dbname = "<?php echo "$dbname"; ?>";<?php if ($dbname == "") { echo "<font color='red'><small>&nbsp;&nbsp;&nbsp;&laquo;&nbsp;EMPTY!!</small></font>"; } ?></td></tr>
				</table>
			</li>

			<li>
				After your config_db.php file is properly configured, you may need to create a new MySQL database: <a href="install.php?indb=1">&raquo; Install Now</a>
				<?php
				if ($indb == "1") {
			...
			<li>
				Once your database is created (or you chose an exiting one), this step will create the necessary tables: <a href="install.php?indb=2">&raquo; Import Now</a>
				<?php
				if ($indb == "2") {
					$con = mysql_connect("$dbhost","$dbuname","$dbpass");
		...
		<br />
		<br /><br /><br />-------------------------------------------------------------------------------------------------------------------------------------
		<br /><span style="color:yellow">&raquo; After you've finished using this script, DELETE the install folder as a security measure!</span>
