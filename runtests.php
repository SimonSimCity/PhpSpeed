...
<head>
	...
	<title>PHPspeed | Run Tests</title>
...
$TESTS_DIRS = array('tests');

if (isset($_GET['test']) && ($_GET['test'] == 'go')) {
	$test = "go";
} else {
	$test = "";
}
...
if ($test != 'go') {
	...
	echo "
<H6>&nbsp;&nbsp;&nbsp;&nbsp;Benchmark # 1 | Synthetic PHP Test</h6>
    <table width=\"94%\" cellpadding=\"10\" cellspacing=\"1\" align=\"center\" class=\"forumline\">
        <tr>
          <td class=\"row2\">
             ...
             synthetic in nature and will test the speed of your PHP install, but not necessarily the
             'real world' performance of your site.  Using this test before and after tweaks to the
             <b>php.ini</b> file and your apache <b>http.conf</b> file will help you determine if your
             tweaks have enhanced your php performance.  You should never rely soley on synthetic tests;
             you also need to use the page loading test to determine if visitors to your website will
             see a benefit from your tweaks.<br /><br />
             ...
             To begin this test click <a href=\"runtests.php?test=go\">&raquo; here</a>
          </td>
        </tr>
    </table>
";
    ...
    VALUES ('','" . time() . "','$cpu_info','$uptime','" . round(($tot_mem/1024),2) . " GB -  $percent_used % Used','{$ver['version']}','" . phpversion() . "','" . mysql_get_server_info() . "','" . $_SERVER['SERVER_SOFTWARE'] . "','" . serialize($results) . "','" . count($results) . "','$base','" . round($total_time) . "','" . round($score) . "','" . $_SERVER['HTTP_HOST'] . "')");
		...
		echo '<h6>&nbsp;&nbsp;&nbsp;Please be patient while the tests are running...</h6>';
		...
		echo "<br /><a href=\"runtests.php?test=go\">&raquo; Run again</a> | <a href=\"benchhistory.php?bench=1\">&raquo; Test history</a></div></div>\n";
