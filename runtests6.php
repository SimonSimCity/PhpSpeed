...
define('MIN_BASE', 5);
////define('DEFAULT_BASE', 250);
//define('MIN_BASE', 50);

$TESTS_DIRS = array('tests5',
'tests4',
'tests3',
'tests2',
'tests');
...
if ($test != 'go') {
	...
	echo "
<H6>&nbsp;&nbsp;&nbsp;&nbsp;Benchmark # 6 | Server Benchmark</h6>
    <table width=\"94%\" cellpadding=\"10\" cellspacing=\"1\" align=\"center\" class=\"forumline\">
        <tr>
          <td class=\"row2\">
            This benchmark throws everything we've got at your server to find out how fast it really is!  This test can be a resource hog, so you
            should not run this test during your server's peak load time.  Testing includes synthetic PHP and MySQLn tests, PHP page loading with
            and without MySQL interaction, and file read/writing within PHP.
            <br /><br />
             ...
             To begin this test click <a href=\"runtests6.php?test=go\">&raquo; here</a>
          </td>
        </tr>
    </table>
";
	...
	if(isset($_SESSION['logged']) && $_SESSION['logged'] == 'yes') {
		...
		function show_summary($base, &$results, $csv_file) {
			...
			$score = (float) $base * 150.0 / $total_time;
    ...
    VALUES ('','" . time() . "','$cpu_info','$uptime','" . round(($tot_mem/1024),2) . " GB -  $percent_used % Used','{$ver['version']}','" . phpversion() . "','" . mysql_get_server_info() . "','" . $_SERVER['SERVER_SOFTWARE'] . "','" . serialize($results) . "','" . count($results) . "','$base','" . round($total_time) . "','" . round($score) . "','" . $_SERVER['HTTP_HOST'] . "')");
		...
		echo "<br /><a href=\"runtests6.php?test=go\">Run again</a> | <a href=\"benchhistory.php?bench=6\">&raquo; Test history</a></div></div>\n";
